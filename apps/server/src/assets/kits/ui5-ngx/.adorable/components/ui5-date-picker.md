...
**Import:** `import { DatePickerComponent } from '@ui5/webcomponents-ngx/main/date-picker';`
**Export As:** `ui5DatePicker`
**Package:** `@ui5/webcomponents` (main)

## Basic Usage
```html
<ui5-date-picker [primaryCalendarType]="..." (ui5Change)="onChange($event)"></ui5-date-picker>
```

## Description
The ui5-date-picker component provides an input field with assigned calendar which opens on user action. The ui5-date-picker allows users to select a localized date using touch, mouse, or keyboard input. It consists of two parts: the date input field and the date picker. The user can enter a date by: - Using the calendar that opens in a popup - Typing it in directly in the input field When the user makes an entry and presses the enter key, the calendar shows the corresponding date. When the user directly triggers the calendar display, the actual date is displayed. If a date is entered by typin
