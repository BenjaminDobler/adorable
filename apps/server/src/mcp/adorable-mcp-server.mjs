...
// apps/server/src/mcp/adorable-mcp-server.ts
import * as readline from "readline";
var SERVER_PORT = process.env["ADORABLE_SERVER_PORT"] || "3333";
var AGENT_PORT = process.env["ADORABLE_AGENT_PORT"] || "3334";
var USER_ID = process.env["ADORABLE_USER_ID"] || "";
var BRIDGE_TOKEN = process.env["ADORABLE_BRIDGE_TOKEN"] || "";
var PROJECT_PATH = process.env["ADORABLE_PROJECT_PATH"] || process.cwd();
var CDP_BASE = `http://localhost:${AGENT_PORT}/api/native/cdp`;
var BRIDGE_BASE = `http://localhost:${SERVER_PORT}/api/internal/bridge`;
function textResult(text, isError = false) {
  return { content: [{ type: "text", text }], isError };
}
function imageResult(base64, mimeType = "image/jpeg") {
  return { content: [{ type: "image", data: base64, mimeType }] };
}
async function cdpCall(endpoint, body = {}) {
...
async function cdpCallJson(endpoint, body = {}) {
  const res = await fetch(`${CDP_BASE}/${endpoint}`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body)
...
async function bridgeCall(path, body = {}, method = "POST") {
  const res = await fetch(`${BRIDGE_BASE}/${path}`, {
    ...
    body: method === "GET" ? void 0 : JSON.stringify({ ...body, userId: USER_ID })
  });
  if (!res.ok) {
...
var cdpTools = [
  ...
  },
  {
    ...
    async handler(args) {
      const result = await cdpCallJson("evaluate", {
        expression: args.selector ? `window.__ong?.getComponentInfo?.('${args.selector}') || document.querySelector('${args.selector}')?.outerHTML?.substring(0, 500)` : `window.__ong?.getComponentTree?.() || 'ONG annotations not available'`
      });
      return textResult(JSON.stringify(result, null, 2));
    }
  },
  ...
  },
  {
    ...
    async handler(args) {
      const result = await cdpCallJson("evaluate", {
        expression: args.action === "clear" ? `document.getElementById('adorable-injected-css')?.remove(); 'Cleared'` : `(() => {
...
var figmaTools = [
  {
    ...
    inputSchema: { type: "object", properties: {} },
    async handler() {
      const result = await bridgeCall("figma/get_selection");
      return textResult(JSON.stringify(result, null, 2));
    }
  },
  {
    ...
    async handler(args) {
      const result = await bridgeCall("figma/get_node", args);
      return textResult(JSON.stringify(result, null, 2));
    }
  },
  {
    ...
    async handler(args) {
      const result = await bridgeCall("figma/export_node", args);
  ...
  },
  {
    ...
    async handler(args) {
      const result = await bridgeCall("figma/select_node", args);
      return textResult(JSON.stringify(result, null, 2));
    }
  },
  {
    ...
    async handler(args) {
      const result = await bridgeCall("figma/search_nodes", args);
      return textResult(JSON.stringify(result, null, 2));
    }
  },
  {
...
var skillTools = [
  ...
  },
  {
    ...
    async handler(args) {
      const result = await bridgeCall("lesson/save", args);
      return textResult(JSON.stringify(result, null, 2));
    }
  }
];
var allTools = [...cdpTools, ...figmaTools, ...skillTools];
