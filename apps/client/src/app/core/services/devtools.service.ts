...
export class DevtoolsService {
  ...
  private buildInspectExpression(toolName: string, args: Record<string, any>): string {
    ...
    switch (toolName) {
      ...
      case 'inspect_routes':
        return `(function(){if(!window.ng||!window.ng.ɵgetRouterInstance)return{error:'router API unavailable'};var root=document.querySelector('[ng-version]')||document.querySelector('app-root');if(!root)return{error:'no root'};var inj=window.ng.getInjector(root);var r=window.ng.ɵgetRouterInstance(inj);if(!r)return{error:'no router'};return{url:r.url,routes:r.config.map(function(c){return{path:c.path,component:c.component?c.component.name:'',lazy:!!c.loadComponent||!!c.loadChildren};})};})()`;
